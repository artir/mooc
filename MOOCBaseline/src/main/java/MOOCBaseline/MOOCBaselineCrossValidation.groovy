...
def trainDir = 'data'+java.io.File.separator+'full'+java.io.File.separator
...
for (Predicate p : [user, post, upvote, downvote, posts, inthread])
{
	println "\t\t\tREADING " + p.getName() +" ...";
	insert = data.getInserter(p, trainPart)
...
//		if (i != trainingTarget && i != fold)
	...
	Database trainDB = data.getDatabase(trainWritePartitions.get(fold),[postActivity, ontime, reputation, polarity, subjective, submitted, user, post, upvote, downvote, posts, viewActivity, voteActivity] as Set, (Partition []) trainPart)
	Database testDB = data.getDatabase(testWritePartitions.get(fold), [postActivity, ontime, reputation, polarity, subjective, submitted, user, post, upvote, downvote, posts, viewActivity, voteActivity] as Set, (Partition []) testReadPartitions.toArray())
